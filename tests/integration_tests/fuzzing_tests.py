...
class TestFuzzing:
    ...
    def fuzzing_test_network(
        ...
        for i in range(n):
            ...
            input = torch.rand(batched_in_shape, device=device) * 2 - 1
    ...
    def test_nn4sys(self) -> None:
        ...
        def get_nn4sys_128d() -> Tuple[AbstractNetwork, Tuple[int, ...]]:
            onnx_path = (
                "vnn-comp-2022-sup/benchmark_vnn22/nn4sys2022/model/mscn_128d.onnx"
            )
        ...
        # self.fuzzing_test_network(
        #     get_nn4sys_128d, prev_interval_to_call(dp_call), 0.01, 1
        # )
        # self.fuzzing_test_network(
        ...
        def forward_to_dp(
            ...
            with torch.no_grad():
                net.set_layer_bounds_via_forward_dp_pass(
                    bs_config, input_lb=bounds[0], input_ub=bounds[1]
                )
