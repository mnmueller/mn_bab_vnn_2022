...
def run_root_planet_lb(domain, verif_layers):
    ...
    # Run lower bounding
    intermediate_net = Propagation(
        verif_layers, type="best_prop", params={"best_among": ["KW", "crown"]}
    )
    with torch.no_grad():
    ...
    prop_net = Propagation(
        verif_layers, type="alpha-crown", params=prop_params, store_bounds_primal=True
    )
    with torch.no_grad():
        prop_net.build_model_using_bounds(
            domain.unsqueeze(0),
            (intermediate_net.lower_bounds, intermediate_net.upper_bounds),
        )
...
def run_quick_attacks(data, model, gpu):
    ...
    for prop in range(10):
        if prop != y:
            adv_examples, is_adv = adv_model.create_adv_examples(
                data, model, return_criterion="one", gpu=gpu, target=prop
            )
...
def binary_eps_search(
    eps_lower_bound, eps_upper_bound, bab_function, quantization=1e-3, mode="LB"
):
    """
    ...
    return_value = (
        math.floor(eps_lower_bound / quantization) * quantization
        if mode == "UB"
        else math.ceil(eps_upper_bound / quantization) * quantization
    )
    return return_value
...
def create_oval21(seed, use_cpu):
    ...
    for onnx_name, nn_name, max_solv_batch in zip(
        onnx_names, nn_names, max_solver_batch_list
    ):
        x, y, th_model, _ = load_cifar_oval_kw_1_vs_all(
            nn_name,
            0,
            max_solver_batch=max_solv_batch,
    ...
    images_indices = []
    for counter, (nn_name, max_solv_batch) in enumerate(
        zip(nn_names, max_solver_batch_list)
    ):
        images_indices.append([])
        while len(images_indices[counter]) < n_images_per_net:
            tentative_indices = torch.randint(
                low=0, high=max_index, size=(n_images_per_net,)
            ).tolist()
    ...
    for counter, (nn_name, onnx_name, max_solv_batch, img_ids) in enumerate(
        zip(nn_names, onnx_names, max_solver_batch_list, images_indices)
    ):
        ...
        for idx in range(n_images_per_net):
            ...
            sat_shift = 2 / 3
            found_epsilon = (
                min(eps_lb, eps_ub) * (1 - sat_shift) + max(eps_lb, eps_ub) * sat_shift
            )

            # Record the results: vnn-lib specification.
            filename = (
                vnnlib_path + f"{nn_name}-img{imag_idx}-eps{found_epsilon}.vnnlib"
            )
            comment = (
                f"Adversarial robustness property for network {nn_name}. l_inf radius: {found_epsilon}, "
                f"CIFAR10 test image n. {imag_idx}."
            )
            _, ground_truth, _, domain = load_cifar_oval_kw_1_vs_all(
                nn_name,
                int(imag_idx),
                epsilon=found_epsilon,
                max_solver_batch=max_solv_batch,
                no_verif_layers=True,
                use_cpu=use_cpu,
            )
    ...
    print(
        f"The dataset creation process took {datasetcreation_end - datasetcreation_start}"
    )


def write_adversarial_robustness_vnnlib(
    ...
    with open(filename, "w") as f:
        ...
        f.write(
            f"; Output constraints (encoding the conditions for a property counter-example):\n"
        )
        f.write(f"(assert (or\n")
        for i in range(n_classes):
            if i != ground_truth:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--seed", type=int, help="Random seed for the image sampling", default=0
    )
    parser.add_argument(
        "--cpu", action="store_true", help="Use cpu instead of a gpu (if not available)"
    )
