...
class Propagation(DualBounding):
    ...
    def __init__(
        ...
        self.params = (
            dict(default_params, **params) if params is not None else default_params
        )
        ...
        if type == "best_prop":
            assert (
                self.params["best_among"] is not None
            ), "Must provide list of prop types to choose best from"
            self.optimize = self.best_prop_optimizers(self.params["best_among"])
        else:
            self.type = type
            self.optimize = self.propagation_optimizer

    def propagation_optimizer(
        self, weights, additional_coeffs, lower_bounds, upper_bounds
    ):
        ...
        if self.type in ["crown", "KW"]:
            ...
            # Compute objective.
            bounding_out = self.compute_bounds(
                weights, add_coeff, dual_vars, lower_bounds, upper_bounds
            )
            ...
            argmin_ibs = torch.where(
                xn_coeff >= 0,
                lower_bounds[lay_n].unsqueeze(1),
                upper_bounds[lay_n].unsqueeze(1),
            )
            ...
            for lay_idx, (lbs, ubs) in enumerate(zip(lower_bounds, upper_bounds)):
                if lay_idx > 0 and lay_idx < len(weights):
                    ...
                    crown_lb_slope = (
                        ...
                        .type(lbs.dtype)
                        .unsqueeze(1)
                        .repeat((1, neuron_batch_size) + ((1,) * (lbs.dim() - 1)))
                    )
            ...
            with torch.enable_grad():
                ...
                optimizer = torch.optim.Adam(
                    optvars,
                    lr=self.params["initial_step_size"],
                    betas=self.params["betas"],
                )
                # Decay step size.
                scheduler = torch.optim.lr_scheduler.ExponentialLR(
                    optimizer, self.params["step_size_decay"]
                )
    ...
    def compute_bounds(
        ...
        x0 = torch.where(
            x0_coeff >= 0, lower_bounds[0].unsqueeze(1), upper_bounds[0].unsqueeze(1)
        )
        ...
        for lay_idx in range(1, len(weights)):
            ...
            neg_bias.masked_fill_(
                ubs == lbs, 0
            )  # cover case in which ubs & lbs coincide
            bound += utils.bdot(dual_vars.lambdas[lay_idx - 1].clamp(0, None), neg_bias)
            bound -= utils.bdot(
                dual_vars.mus[lay_idx - 1], weights[lay_idx - 1].get_bias()
            )
    ...
    def get_lower_bound_network_input(self):
        ...
        assert self.bounds_primal.shape[1] in [
            1,
            2,
        ], "the last layer must have a single neuron"
        l_0 = self.input_domain.select(-1, 0)
        u_0 = self.input_domain.select(-1, 1)
        net_input = (1 / 2) * (u_0 - l_0) * self.bounds_primal.select(
            1, self.bounds_primal.shape[1] - 1
        ) + (1 / 2) * (u_0 + l_0)
        return net_input
...
class PropDualVars:
    ...
    def get_duals_from(
        ...
        mus, mu, lay_idx = handle_propagation_add_coeff(
            weights, additional_coeffs, lower_bounds
        )
        ...
        while lay_idx > 0:
            ...
            if do_crown:
                ...
                lb_slope = torch.where(
                    (lbs < 0) & (ubs > 0), alphas[lay_idx - 1], ub_slope
                )
