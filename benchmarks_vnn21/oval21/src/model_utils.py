...
def cifar_kw_net_loader(model):
    ...
    if model == "cifar_base_kw":
        ...
        model.load_state_dict(
            torch.load(model_name, map_location="cpu")["state_dict"][0]
        )
...
def load_cifar_oval_kw_1_vs_all(
    model,
    idx,
    epsilon=2 / 255,
    ...
    domain = torch.stack(
        [normalizer((x - epsilon).clamp(0, 1)), normalizer((x + epsilon).clamp(0, 1))],
        dim=-1,
    )
    ...
    verification_layers = one_vs_all_from_model(
        model, y, domain=domain, max_solver_batch=max_solver_batch, use_cpu=use_cpu
    )
    return x, y, verification_layers, domain


def one_vs_all_from_model(
    model, true_label, domain=None, max_solver_batch=10000, use_cpu=False
):
    """
