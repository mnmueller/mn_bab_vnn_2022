...
def compute_output_padding(l_0, layer):
    ...
    if (
        l_0.shape[2] + 2 * layer.padding[0] - layer.dilation[0] * (w_1.shape[2] - 1) - 1
    ) % layer.stride[0] != 0:
        h_out_0 = math.floor(
            (
                l_0.shape[2]
                + 2 * layer.padding[0]
                - layer.dilation[0] * (w_1.shape[2] - 1)
                - 1
            )
            / layer.stride[0]
            + 1
        ...
        output_padding_0 = (
            ...
            + 2 * layer.padding[0]
            - layer.dilation[0] * (w_1.shape[2] - 1)
            - (h_out_0 - 1) * layer.stride[0]
        )
    if (
        l_0.shape[3] + 2 * layer.padding[1] - layer.dilation[1] * (w_1.shape[3] - 1) - 1
    ) % layer.stride[1] != 0:
        h_out_1 = math.floor(
            (
                l_0.shape[3]
                + 2 * layer.padding[1]
                - layer.dilation[1] * (w_1.shape[3] - 1)
                - 1
            )
            / layer.stride[1]
            + 1
...
class LinearOp:
    ...
    def forward(self, inp):
        ...
        forw_out = torch.bmm(
            ...
            self.weights.t()
            .unsqueeze(0)
            .expand((inp.shape[0], self.weights.shape[1], self.weights.shape[0])),
        )
    ...
    def interval_forward(self, lb_in, ub_in):
        ...
        lb_out = (
            pos_lay.forward(lb_in.unsqueeze(1)) + neg_lay.forward(ub_in.unsqueeze(1))
        ).squeeze(1)
        ub_out = (
            pos_lay.forward(ub_in.unsqueeze(1)) + neg_lay.forward(lb_in.unsqueeze(1))
        ).squeeze(1)
    ...
    def backward(self, out):
        ...
        back_inp = torch.bmm(
            out,
            self.weights.unsqueeze(0).expand(
                (out.shape[0], self.weights.shape[0], self.weights.shape[1])
            ),
        )

        if self.flatten_from_shape is not None:
            back_inp = back_inp.view(
                (out.shape[0], out.shape[1]) + self.flatten_from_shape
            )
        return back_inp
    ...
    def subset_backward(self, out, subset):
        ...
        weights = torch.gather(
            self.weights.unsqueeze(0).expand(
                subset.shape[0], self.out_features, self.in_features
            ),
            1,
            subset.unsqueeze(-1).expand(
                subset.shape[0], subset.shape[1], self.weights.shape[-1]
            ),
        )
        back_inp = torch.bmm(out, weights)
...
class BatchLinearOp(LinearOp):
    ...
    def forward(self, inp):
        ...
        forw_out = (
            (self.weights.unsqueeze(1) @ inp.unsqueeze(-1))
            .view((domain_batch, layer_batch, self.weights.shape[1], 1))
            .squeeze(-1)
        )
    ...
    def subset_backward(self, out, subset):
        ...
        weights = torch.gather(
            ...
            subset.unsqueeze(1)
            .unsqueeze(-1)
            .expand(subset.shape[0], 1, subset.shape[1], self.weights.shape[-1]),
        )
    ...
    def subset_backward_non_batch(self, out, subset):
        back_inp = (
            out.unsqueeze(1) @ self.weights.unsqueeze(1)[:, :, subset, :]
        ).squeeze(1)
        if self.flatten_from_shape is not None:
            back_inp = back_inp.view(
                (out.shape[0], out.shape[1]) + self.flatten_from_shape
    ...
    def __repr__(self):
        return (
            f"{self.batch_size} x <Linear: {self.in_features} -> {self.out_features}>"
        )


class ConvOp:
    def __init__(
        self, weights, bias, stride, padding, dilation, groups, output_padding=0
    ):
        self.weights = weights
        if bias.dim() == 1:
    ...
    def interval_forward(self, lb_in, ub_in):
        ...
        unbiased_lb_out = F.conv2d(
            lb_in, pos_wts, None, self.stride, self.padding, self.dilation, self.groups
        ) + F.conv2d(
            ub_in, neg_wts, None, self.stride, self.padding, self.dilation, self.groups
        )
        unbiased_ub_out = F.conv2d(
            ub_in, pos_wts, None, self.stride, self.padding, self.dilation, self.groups
        ) + F.conv2d(
            lb_in, neg_wts, None, self.stride, self.padding, self.dilation, self.groups
        )
    ...
    def unfold_input(self, inp, gather=None):
        ...
        if fold_back:
            unfolded_inp = unfolded_inp.view(
                domain_batch_size, batch_size, *unfolded_inp.shape[1:]
            )
        if gather is not None:
            ...
            unfolded_inp = unfolded_inp.expand(
                ((domain_batch_size, batch_size) + unfolded_inp.shape[2:])
            )
            unfolded_inp = unfolded_inp.gather(
                -1,
                gather[1]
                .unsqueeze(-2)
                .expand(
    ...
    def unfold_weights(self, gather=None):
        ...
        if gather is not None:
            ...
            unfolded_weights = unfolded_weights.expand(
                batch_shape + unfolded_weights.shape
            )
            unfolded_weights = unfolded_weights.gather(
                -2, gather[0].unsqueeze(-1).expand((gather[0].shape + (kernel_shape,)))
            )
    ...
    def unfold_output(self, out, gather=None):
        ...
        If gather is available, returns only selected indices of the unfolded vector (contained in the gather tensor).
        """
        batch_channel_shape = out.shape[
            :3
        ]  # linearize everything that's not batches or channels
        ...
        if gather is not None:
            ...
            unfolded_out = (
                unfolded_out.view((*out.shape[:2], -1))
                .gather(-1, gather[2])
                .view((*out.shape[:2], restr_channel, restr_spatial))
            )
        return unfolded_out
    ...
    def fold_unfolded_input(self, unfolded_inp, folded_inp_spat_shape):
        ...
        if fold_back:
            folded_inp = folded_inp.view(
                domain_batch_size, batch_size, *folded_inp.shape[1:]
            )
    ...
    def zero_scatter_folded_out(self, out, indices):
        ...
        out_vector.view((*out.shape[:2], -1)).scatter_(
            -1, extended_indices, unfolded_out.gather(-1, extended_indices)
        )
        return out_vector
    ...
    if not is_full_batch:
        if is_part_batch:
            ...
            if end_batch_index <= fixed_len:
                ...
                slice_coeffs = torch.zeros(
                    (batch_size, c_neuron_list.shape[1], nb_out),
                    device=tensor_example.device,
                    dtype=tensor_example.dtype,
                )
                slice_coeffs.scatter_(2, c_neuron_list.unsqueeze(2), -1)
            elif start_batch_index >= fixed_len:
                # Only lower bounds
                c_neuron_list = node[1][
                    :, start_batch_index - fixed_len : end_batch_index - fixed_len
                ]
                slice_coeffs = torch.zeros(
                    (batch_size, c_neuron_list.shape[1], nb_out),
                    device=tensor_example.device,
            ...
            slice_coeffs = slice_coeffs.view(
                (
                    batch_size,
                    slice_coeffs.size(1),
                )
            ...
            if tensor_example.dim() == 2:
                ...
                slice_coeffs[:, 0, node[1][0], node[1][1], node[1][2]] = (
                    -1 if upper_bound else 1
                )
            else:
...
class OptimizationTrace:
    ...
    def get_last_layer_bounds_means_trace(
        self, first_half_only_as_ub=False, second_half_only=False
    ):
        """
        ...
        if first_half_only_as_ub:
            if self.bounds_progress_per_layer[last_layer][0].dim() > 1:
                bounds_trace = [
                    -bounds[:, : int(bounds.shape[1] / 2)].mean()
                    for bounds in self.bounds_progress_per_layer[last_layer]
                ]
            else:
                bounds_trace = [
                    -bounds[: int(len(bounds) / 2)].mean()
                    for bounds in self.bounds_progress_per_layer[last_layer]
                ]
            ...
            bounds_trace = [
                bounds.mean() for bounds in self.bounds_progress_per_layer[last_layer]
            ]
        return bounds_trace
