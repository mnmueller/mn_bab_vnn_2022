...
def generateNBenchmarks(
    network, epsilon, numBenchmarks, timeout, csvFile, specsDir, seed
):
...
def getBenchmark(
    network, test_images, test_labels, epsilon, index, timeout, csvFile, specsDir
):
    ...
    vnnLibFile = "{}/network{}_index{}_eps{}_target{}_orig{}.vnnlib".format(
        specsDir,
        os.path.basename(network).split(".")[0],
        index,
        epsilon,
        targetLabel,
        label,
    ...
    f = open(vnnLibFile, "w")
    print(
        "; index {}, correct label {}, target label {}, epsilon {}".format(
            index, label, targetLabel, epsilon
        ),
        file=f,
    )
...
def main():
    ...
    parser.add_argument(
        "--networks", type=str, default="./nets/", help="The network directory"
    )
    parser.add_argument(
        "--specs", type=str, default="./specs", help="The spec directory"
    )
    parser.add_argument(
        "--timeout",
        type=int,
        default=300,
        help="Number of benchmarks per network-epsilon pair",
    )
    parser.add_argument(
        "--csv",
        type=str,
        default="./marabou-cifar10_instances.csv",
