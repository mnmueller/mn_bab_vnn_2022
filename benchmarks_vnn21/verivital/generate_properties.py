...
def get_data(img_num: int = 20, seed: int = 0):
    ...
    while image_counter < img_num:
        ...
        if (
            output_avg != labels[i : i + 1].item()
            or output_max != labels[i : i + 1].item()
        ):
            correctly_classified = False

        if (
            output_avg == labels[i : i + 1].item()
            and output_max == labels[i : i + 1].item()
        ):
            correctly_classified = True
            # print(i)
            image_counter += 1
...
def write_vnnlib_spec(
    upper_bound: torch.Tensor, lower_bound: torch.Tensor, correct_label: int, path: str
):
...
def csv_instances(num_props: int = 25, path: str = "verivital_instances.csv"):
    ...
    nets = ["Convnet_avgpool.onnx", "Convnet_maxpool.onnx"]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Specification Generrator: vnnlib format",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "--seed", type=int, default=0, help="seed is selected for image selection"
    )
