...
def attack_pgd(
    ...
    for n in range(num_restarts):
        ...
        delta = torch.max(
            torch.min(delta, sample_upper_limit), sample_lower_limit
        ).requires_grad_()
        ...
        for _ in range(attack_iters):
            ...
            if not multi_targeted:
                ...
                output = model(inputs.view(-1, *input_shape[1:])).view(
                    input_shape[0], num_classes - 1, num_classes
                )
            if not multi_targeted:
                # Untargeted attack on logit space.
                runnerup = (
                    output.scatter(dim=1, index=y.view(-1, 1), value=-100.0).max(1)[0]
                    + 0.01
                )
            ...
            with torch.no_grad():
                ...
                if not multi_targeted:
                    ...
                    delta_targeted = delta.gather(
                        dim=1,
                        index=indices.view(-1, 1, 1, 1, 1).expand(
                            -1, -1, *input_shape[1:]
                        ),
                    ).squeeze(1)
                    best_delta[all_loss >= best_loss] = delta_targeted[
                        all_loss >= best_loss
                    ]
...
class AdamClipping(Optimizer):
    ...
    @torch.no_grad()
    def step(
        self, clipping=None, lower_limit=None, upper_limit=None, sign=None, closure=None
    ):
