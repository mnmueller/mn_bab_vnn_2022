...
cifar10_mean = (
    ...
    0.4822,
    0.4465,
)  # np.mean(train_set.train_data, axis=(0,1,2))/255
cifar10_std = (0.2471, 0.2435, 0.2616)  # np.std(train_set.train_data, axis=(0,1,2))/255


def load_data(
    data_dir: str = "./tmp", num_imgs: int = 25, random: bool = False
) -> tuple:
...
# noinspection PyShadowingNames
def save_vnnlib(
    input_bounds: torch.Tensor, label: int, spec_path: str, total_output_class: int = 10
):
...
def create_csv():
    ...
    for i in range(48):
        instance_list.append(
            [
                ...
                f"vnnlib_properties_pgd_filtered/resnet2b_pgd_filtered/prop_{i}_eps_0.008.vnnlib",
                "300",
            ]
        )
    ...
    for i in range(24):
        instance_list.append(
            [
                f"onnx/{model_name}.onnx",
                f"vnnlib_properties_pgd_filtered/resnet4b_pgd_filtered/prop_{i}_eps_0.004.vnnlib",
                "300",
            ]
        )
...
def create_vnnlib(args):
    ...
    print(
        f"===== model: {args.model} epsilons: {args.epsilons} total images: {args.num_images} ====="
    )
    ...
    result_dir = os.path.join(
        "../vnnlib_properties_pgd_filtered/", args.model + "_pgd_filtered/"
    )
    ...
    for eps in epsilons:
        ...
        for i in range(images.shape[0]):
            ...
            assert (attack_image >= 0.0).all()
            assert (attack_image <= 1.0).all()
            ...
            if attack_label != label:
                print(
                    "pgd succeed image {}, label {}, against label {}".format(
                        i, label, attack_label
                    )
                )
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--deterministic",
        action="store_true",
        help="Do not generate random examples; use dataset order instead.",
    )
    ...
    parser.add_argument(
        ...
        choices=["cpu", "gpu"],
        default="cpu",
        help="Choose device to generate adversarial examples.",
    )
