...
def clean_acc(model_name, images, labels):
    ...
    model.load_state_dict(
        torch.load(os.path.join(model_name, "model_best.pth"))["state_dict"]
    )
...
def main():
    ...
    print(
        "ResNet-2B clean acc: {} out of {}".format(
            clean_acc("resnet2b", images, labels), num_imgs
        )
    )
    print(
        "ResNet-4B clean acc: {} out of {}".format(
            clean_acc("resnet4b", images, labels), num_imgs
        )
    )
