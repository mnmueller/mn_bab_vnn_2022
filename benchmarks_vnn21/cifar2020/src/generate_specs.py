...
def write_vnn_spec(
    ...
    if np.all(mean == 0.0) and np.all(std == 1.0):
        ...
        competing_norm_ids = [
            int(
                re.match(
                    f"{prefix}_idx_{index}_eps_{eps:.5f}_n([0-9]+).vnnlib", spec
                ).group(1)
            )
            for spec in existing_specs
            if spec.startswith(f"{prefix}_idx_{index}_eps_{eps:.5f}_n")
        ]
...
def main():
    parser = argparse.ArgumentParser(
        description="VNN spec generator",
    ...
    parser.add_argument(
        "--n", type=int, default=25, help="The number of specs to generate"
    )
    parser.add_argument(
        "--block", action="store_true", default=False, help="Generate specs in a block"
    )
    parser.add_argument(
        "--seed", type=int, default=42, help="Random seed for idx generation"
    )
    parser.add_argument(
        "--start_idx",
        type=int,
    ...
    parser.add_argument(
        "--network", type=str, default=None, help="Network to evaluate as .onnx file."
    )
    parser.add_argument(
        "--instances",
        type=str,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Overwrite old instances.csv",
    )
    parser.add_argument(
        "--mean",
        nargs="+",
    ...
    spec_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "../specs", args.dataset
    )
    ...
    with open(args.instances, "w" if args.new_instances else "a") as f:
        while i < len(idxs):
            ...
            if args.network is None or all(y == y_pred):
                spec_i = write_vnn_spec(
                    dataset,
                    idx,
                    args.epsilon,
                    dir_path=spec_path,
                    prefix=args.dataset + "_spec",
                    data_lb=0,
                    data_ub=1,
                    n_class=10,
                    mean=mean,
                    std=std,
                    negate_spec=args.negate_spec,
                )
                f.write(
                    f"{''if args.network is None else os.path.join('nets',os.path.basename(args.network))},{os.path.join('specs',args.dataset,spec_i)},{args.time_out:.1f}\n"
                )
                ...
                if len(idxs) < len(
                    ...
                    if (
                        args.block
                    ):  # if we want samples in a block, just get the next one
                        idxs.append(
                            *get_sample_idx(
                                1, True, n_max=len(dataset), start_idx=idxs[-1] + 1
                            )
                        )
                    else:  # otherwise sample deterministicly (for given seed) until we find a new sample
                        tmp_idx = get_sample_idx(
                            1, False, seed=args.seed + ii, n_max=len(dataset)
                        )
                        ii += 1
                        ...
                        idxs.append(*tmp_idx)
        print(
            f"{len(idxs)-n_ok} samples were misclassified{''if args.dont_extend else ' and replacement samples drawn'}."
        )
