...
from generate_specs import get_cifar10, get_mnist, get_sample_idx
...
def main():
    ...
    parser.add_argument(
        "--network", type=str, required=True, help="Network to evaluate as .onnx file."
    )
    parser.add_argument(
        "--debug", action="store_true", default=False, help="Print more."
    )
    parser.add_argument(
        "--mean",
        nargs="+",
    ...
    if args.start_idx is not None:
        args.block = True
        print(
            f"Producing deterministic {args.n} indices starting from {args.start_idx}."
        )
    ...
    for idx in idxs:
        ...
        if args.debug:
            print(
                f"Sample idx {idx}, predicted class: {int(np.argmax(pred_onx,axis=-1))}, true class {y}:\n",
                pred_onx,
            )
