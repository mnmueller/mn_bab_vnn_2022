...
) -> List[Tuple[Tensor, Tensor, Tuple[Tensor, Tensor, Tensor], int]]:
    n_splits = min(
        domain_splitting.initial_splits,
        int(domain_splitting.batch_size ** (1 / input_ub[0].numel()) + 0.5),
    )
...
def batch_splits(
    queue: List[Tuple[Tensor, Tensor, Tuple[Tensor, Tensor, Tensor], int]],
    batch_size: int,
...
def split_input_regions(
    ...
    for specLB, specUB in input_regions:
        ...
        for i in range(di):
            ...
            assert (specLB[:, d] >= d_lb - 1e-9).all()
            assert (specUB[:, d] <= d_ub + 1e-9).all()
