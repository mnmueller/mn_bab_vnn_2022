...
class MNBaBVerifier:
    ...
    def _verify_query_with_deep_poly(
        ...
        assert isinstance(deep_poly_lbs, Tensor)
        assert isinstance(ub_on_minimum, Tensor)
        verified: Tensor = deep_poly_lbs > early_stopping_thresholds
        falsified: Tensor = ub_on_minimum < early_stopping_thresholds
        return (
            verified,
            falsified,
    ...
    def _verify_with_input_domain_splitting(
        ...
        List[
            Tuple[
                Tensor, Optional[Tensor], Optional[Tuple[Tensor, Tensor, Tensor]], int
            ]
        ],
        Tensor,
    ]:
        num_classes = self.network.output_dim[-1]
        out_queue: List[Tuple[Tensor, Tensor, Tuple[Tensor, Tensor, Tensor], int]] = []
        ...
        query_lb_global = (
            torch.ones([1], dtype=input_lb.dtype, device=input_lb.device) * torch.inf
        )

        queue: List[
            Tuple[Tensor, Tensor, Tuple[Tensor, Tensor, Tensor], int]
        ] = compute_initial_splits(
            input_lb,
            input_ub,
            property_matrix,
        ...
        while len(queue) > 0:
            ...
            if adv_found.any():
                ...
                return [(adv_example, None, None, -1)], query_lb_global
    ...
    def verify_via_config(  # noqa C901 # ignore too complex function
        ...
        if self.outer.forward_dp_pass:
            with torch.no_grad():
                self.network.set_layer_bounds_via_forward_dp_pass(
                    self.config.backsubstitution, input_lb=input_lb, input_ub=input_ub
                )
        ...
        if self.outer.adversarial_attack:
            ...
            if adversarial_example is not None:
                ...
                return (False, adversarial_example, None, None, None)
            ...
            for gt_tuple in const_list:
                ...
                time_budget = max(
                    ...
                    int(
                        2
                        * time_remaining
                        / max((verification_target - verified_so_far + 1), 6)
                        * remaining
