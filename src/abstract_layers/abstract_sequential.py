...
class Sequential(AbstractContainerModule):
    ...
    def propagate_interval(
        ...
        for layer in self.layers:
            ...
            if type(layer) in ActivationLayers or not activation_layer_only:
    ...
    def forward_pass(
        ...
        for i, layer in list(enumerate(self.layers)):
            if type(layer) in ActivationLayers:
                ...
                print(
                    f"ID: {id(layer)} Pre: {pre_unstable:.0f} Post: {post_unstable:.0f} Pre-Width: {pre_width} Post-Width: {post_width}"
                )
                ibp_call()
