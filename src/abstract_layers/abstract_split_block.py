...
class SplitBlock(concreteSplitBlock, AbstractContainerModule):
    ...
    def backsubstitute_shape(
        ...
        res_lower, res_upper = self.res_lower.unsqueeze(0), self.res_upper.unsqueeze(0)
        ...
        div_factors = (
            1 / res_lower.sum(dim=self.outer_reduce_dim + 1),
            1 / res_upper.sum(dim=self.outer_reduce_dim + 1),
        )
    ...
    def propagate_interval(
        ...
        self.res_lower = res_lower[0]  # Don't store interval batch dim
        self.res_upper = res_upper[0]
    ...
    def _get_multiplication_slopes_and_intercepts(
        ...
        convex_lb_slope = (mult_ub_lb - mult_lb_lb) / (input_ub - input_lb + 1e-6)
        convex_lb_intercept = mult_lb_lb - input_lb * convex_lb_slope - 1e-6

        convex_ub_slope = (mult_ub_ub - mult_lb_ub) / (input_ub - input_lb + 1e-6)
        convex_ub_intercept = mult_lb_ub - input_lb * convex_ub_slope + 1e-6
