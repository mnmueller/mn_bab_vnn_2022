...
class Slice(concreteSlice, AbstractModule):
    ...
    def _backsub_affine_form(
        ...
        affine_shape = affine_form.coef.shape
        new_coef_shape = (affine_shape[0], affine_shape[1], *self.input_dim)

        slice_indices = [slice(0, dim, 1) for dim in affine_shape[:2]] + [
            slice(0, dim, 1) for dim in self.input_dim
